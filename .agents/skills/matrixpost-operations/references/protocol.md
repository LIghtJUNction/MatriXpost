...
For a video request, the runner treats WeChat Channels link metadata as local browser input only. A nonempty `sphProductId` is the effective product identifier and takes precedence over `sphLink`, including `sphLink.type:"none"`. Otherwise `sphLink.type:"none"` performs no product action, while `sphLink.type:"product"` requires a nonempty `value`; absent link metadata also performs no product action. Link metadata for other platforms is not acted on by this runner. The WeChat product selection has a finite local wait and runs inside the same temporary attached-browser session as the video workflow. Its local completion never confirms that WeChat accepted, reviewed, published, or retained the association.

`matrixpost accounts` preserves persisted account fields and adds only a safe `readiness` label. It probes only a matching explicit loopback TCP provider runner; no matching runner yields `unavailable`. The runner's `/v1/account-status` endpoint accepts only `{version,platform}` and is disabled unless started with `--allow-account-status-probe` plus valid loopback WebDriver and browser-debugger endpoints. When enabled, it temporarily navigates the already-attached browser to the profile upload page, checks only for its configured upload-file selector, then closes the session. Results are `ready`, `not_ready`, `unavailable`, or `rejected`; they never assert login, inspect cookies, or expose URLs, paths, credentials, or browser state.

`matrixpost review-status --title TITLE` and MCP `review_fanqie_status` query only a matching explicit `fqsp=tcp:127.0.0.1:PORT` provider runner. The versioned `/v1/review-status` request is strictly `{version,platform:"fqsp",title_query}`; its title query is bounded to 200 UTF-8 bytes and is never returned. The endpoint is disabled unless the runner starts with `--allow-review-status-probe` plus valid loopback WebDriver and browser-debugger endpoints. With that opt-in it attaches a temporary session to the user-managed browser, opens Fanqie's documented video-list page, examines only `.video-card`, `.video-card-title`, and `.video-status` internally over at most six bounded scroll attempts, returns only `published`, `under_review`, `rejected`, `not_found`, or `unavailable`, then closes the session. It never returns a title, URL, selector, card text, ID, cookie, credential, or raw error. `published` means only that a matching list card displayed a published-like state; it never proves a remote publication request was accepted, processed, or publicly available.
